...
class HalfRankKReducer(Reducer):
    ...
    def compress(self, tensor, name=None, step=0, **kwargs):
        """
        对单个tensor执行完整PowerSGD压缩（每步都完整计算p和q）。
 
        修改说明：
          原版使用奇偶步交替只传p或q之一，导致重构时p和q来自不同step，
          staleness在训练初期梯度变化剧烈时造成重构误差极大，loss无法下降。
          修改为每步都完整执行一次power iteration（计算p和q），
          通信量变为原来的2倍，但梯度质量有保障。
        """
        ...
        # 保存原始梯度（已包含残差），用于后续更新residual
        original_matrix = matrix.clone()
 
        # 初始化p/q内存（首次或rank切换后重建）
        ...
        with torch.no_grad():
            # 第一步：用当前q（warm-start）投影，计算p = M @ q
            orthogonalize(q)
            torch.matmul(matrix, q, out=p)
 
            # 第二步：用刚算出的p投影，计算q = M^T @ p
            orthogonalize(p)
            torch.matmul(matrix.t(), p, out=q)
 
            # 保存更新后的p/q供warm-start下一步使用
            self.p_memory[key] = p
            self.q_memory[key] = q
 
        # 保存原始梯度供decompress更新residual时使用
        self.last_input[key] = original_matrix
 
        # 同时返回p和q，调用方需要allreduce两者
